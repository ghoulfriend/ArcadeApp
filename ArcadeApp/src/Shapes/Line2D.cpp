...
Line2D Line2D::Spin(float angle) const
{
	Line2D Seg1(Midpoint(), mP0);
	Line2D Seg2(Midpoint(), mP1);

	Vec2D Seg1rotatePoint = mP0.RotationResult(angle, Midpoint());
	Vec2D Seg2rotatePoint = mP1.RotationResult(angle, Midpoint());

	Line2D spunLine(Seg1rotatePoint, Seg2rotatePoint);
